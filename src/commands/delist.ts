  ...
  async deleteServer(ctx: CommandContext) {
    ...
    try {
      ...
      if ("error" in serverListing && serverListing.error) {
        ...
        const notFoundText = "❌ **Not Listed:** This server is not currently registered on Stoward.";

        return await processingMessage.edit({ content: "‎", embeds: [notFoundEmbed.toJSON()] }).catch(async (err) => {
    ...
    try {
      ...
      if ("error" in backendResponse && backendResponse.error) {
        ...
        return await processingMessage.edit({ content: "‎", embeds: [errorEmbed.toJSON()] }).catch(async (err) => {
          if (err.toString().includes("403")) {
      ...
      return await processingMessage.edit({ content: "‎", embeds: [successEmbed.toJSON()] }).catch(async (err) => {
        if (err.toString().includes("403")) {
      ...
      return await processingMessage.edit({ content: "‎", embeds: [failureEmbed.toJSON()] }).catch(async (embedErr) => {
